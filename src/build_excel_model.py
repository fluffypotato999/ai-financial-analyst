...
def _forecast_periods(
    ...
    state = {
        "cash": cash_0, "ar": ar_0, "inv": inv_0,
        "other_assets": other_assets,
        "ap": ap_0, "deferred_rev": dr_0, "other_liab": other_liab,
        "debt": debt, "equity": eq_0,
    }
    ...
    for _ in range(n_fcst):
        ...
        periods.append({
            "Revenue": revenue_t, "CostOfRevenue": cogs_t, "GrossProfit": gp_t,
            "OperatingExpenses": opex_t, "OperatingIncome": op_inc_t,
            "InterestExpense": interest_t, "PreTaxIncome": pretax_t,
            "Tax": tax_t, "NetIncome": ni_t,
            "Cash": cash_t, "AccountsReceivable": ar_t,
            "Inventory": inv_t, "OtherAssets": state["other_assets"],
            "TotalAssets": ta_t,
            "AccountsPayable": ap_t, "DeferredRevenue": dr_t,
            "Debt": new_debt, "OtherLiabilities": state["other_liab"],
            "TotalLiabilities": tl_t, "TotalEquity": eq_t,
            "BalanceCheck": bc_t,
            "OCF": ocf_t, "CapEx": capex_t, "InvestingCF": inv_cf_t,
            "Buybacks": buybacks_t, "DebtAmort": debt_amort,
            "FinancingCF": fin_cf_t, "NetCashChange": net_change,
            "BegCash": state["cash"], "EndCash": cash_t,
            "SBC": sbc_t, "Depreciation": dep_rate,
        })

        # Advance state
        state = {
            "cash": cash_t, "ar": ar_t, "inv": inv_t,
            "other_assets": state["other_assets"],
            "ap": ap_t, "deferred_rev": dr_t, "other_liab": state["other_liab"],
            "debt": new_debt, "equity": eq_t,
        }
...
def _build_cover(ws: Any, ticker: str, company_name: str) -> None:
    ...
    _sc(ws, 1, 2, f"{company_name} ({ticker}) — Three-Statement Financial Model",
        font=Font(bold=True, size=16, name="Calibri"))
    _sc(ws, 2, 2, f"Generated: {today}", font=_FONT_NORM)
    _sc(ws, 3, 2, "Source: SEC EDGAR XBRL — see Sources sheet for accession numbers.",
        font=_FONT_NORM)
...
def _build_scenarios(
    ...
    for row_idx, (key, label, fmt) in enumerate(params, start=2):
        ...
        for col_idx, scen in enumerate(["Base", "Bull", "Bear"], start=2):
            ...
            _sc(ws, row_idx, col_idx, val, font=_FONT_INP,
                fill=_FILL_INPUT, fmt=fmt, halign="right")

    _sc(ws, len(params) + 3, 1,
...
def _build_assumptions(
    ...
    params: list[tuple[str, str, str, str]] = [
        ...
        ("opex_growth_qoq",      "OpEx Growth QoQ",            _FMT_PCT,
         "Quarterly opex growth rate"),
        ("capex_pct_of_revenue", "CapEx % of Revenue",         _FMT_PCT,
        ...
        ("tax_rate",             "Effective Tax Rate",          _FMT_PCT,
         "Applied to positive PreTax Income only"),
        ("debt_amortization_qoq","Debt Amortization Qtrly",    _FMT_USD,
    ...
    for key, label, fmt, note in params:
        ...
        _sc(ws, row, 2, base_assumptions.get(key, 0.0),
            font=_FONT_INP, fill=_FILL_INPUT, fmt=fmt)
...
def _build_income_statement(
    ...
    for i in range(_N_HIST):
        ...
        col = _COL_DATA_START + i
        if rev > 0:
            _sc(ws, gm_row, col, gp / rev, font=_FONT_HIST,
                fill=_FILL_HIST, fmt=_FMT_PCT)
    for j, p in enumerate(fcst_base):
        rev = p.get("Revenue", 1.0) or 1.0
        gp  = p.get("GrossProfit", 0.0) or 0.0
        col = _COL_FCST_START + j
        _sc(ws, gm_row, col, gp / rev, font=_FONT_FCST,
            fill=_FILL_FCST, fmt=_FMT_PCT)

    _sc(ws, 1, _COL_FCST_START - 1,
...
def _build_cash_flow(
    ...
    for label, src_df, col_key in ocf_rows:
        ...
        fcst_vals = [p.get(
            "Depreciation" if col_key == "Depreciation" else
            "SBC"          if col_key == "StockBasedCompensation" else col_key,
            0.0,
        ) for p in fcst_base]
        _write_data_row(ws, row, label, hist_vals, fcst_vals)
        row += 1
    ...
    delta_ar_hist = [ar_series[i] - ar_series[i - 1] if i > 0 else 0.0 for i in range(len(ar_series))]
    delta_ar_fcst = [p.get("AccountsReceivable", 0.0) - (
        fcst_base[j - 1].get("AccountsReceivable", 0.0) if j > 0 else ar_series[-1] if ar_series else 0.0
    ) for j, p in enumerate(fcst_base)]
    _write_data_row(ws, row, "Δ Accounts Receivable (−increase)",
                    [-v for v in delta_ar_hist], [-v for v in delta_ar_fcst])
    row += 1

    if has_inv:
        inv_series = _series(hist_bs, "Inventory")
        delta_inv_hist = [inv_series[i] - inv_series[i - 1] if i > 0 else 0.0 for i in range(len(inv_series))]
        delta_inv_fcst = [p.get("Inventory", 0.0) - (
            fcst_base[j - 1].get("Inventory", 0.0) if j > 0 else inv_series[-1] if inv_series else 0.0
        ) for j, p in enumerate(fcst_base)]
        _write_data_row(ws, row, "Δ Inventory (−increase)",
                        [-v for v in delta_inv_hist], [-v for v in delta_inv_fcst])
        row += 1

    ap_series = _series(hist_bs, "AccountsPayable")
    delta_ap_hist = [ap_series[i] - ap_series[i - 1] if i > 0 else 0.0 for i in range(len(ap_series))]
    delta_ap_fcst = [p.get("AccountsPayable", 0.0) - (
        fcst_base[j - 1].get("AccountsPayable", 0.0) if j > 0 else ap_series[-1] if ap_series else 0.0
    ) for j, p in enumerate(fcst_base)]
    _write_data_row(ws, row, "Δ Accounts Payable (+increase)",
                    delta_ap_hist, delta_ap_fcst)
    row += 1
...
def _build_debt_schedule(
    ...
    _sc(ws, 2, 1, "(Simplified — Debt is not directly available as XBRL line item; "
    ...
    items = [
        ...
        ("Closing Debt Balance", [max(0.0, debt_opening - amort * (i + 1)) for i in range(_N_FCST)]),
...
def _build_summary(
    ...
    _sc(ws, 1, 1, f"{ticker} — Key Metrics Summary",
        font=Font(bold=True, size=14, name="Calibri"))
    ...
    for j in range(_N_HIST):
        _sc(ws, row, _COL_DATA_START + j, f"H{j + 1}",
            font=_FONT_HDR, fill=_FILL_HEADER, halign="center")
    for j in range(_N_FCST):
        _sc(ws, row, _COL_FCST_START + j, f"Q{j + 1}(F)",
            font=_FONT_HDR, fill=_FILL_HEADER, halign="center")
    row += 1
...
def _build_sources(ws: Any, sources_df: pd.DataFrame) -> None:
    ...
    headers = ["line_item", "period_end", "fiscal_year",
               "fiscal_period", "value", "accession_no", "filing_url"]
    for col_idx, h in enumerate(headers, start=1):
...
def _build_revenue_disaggregation(ws: Any, ticker: str) -> None:
    ...
    lines = [
        (1, f"{ticker} Revenue Disaggregation — ASC 280 Note",
         Font(bold=True, size=14, name="Calibri")),
        (3, "SEGMENT REPORTING (ASC 280)",
         Font(bold=True, name="Calibri", size=11)),
        (4, f"{ticker} reports as ONE operating and reportable segment under ASC 280. "
           "There are no separately reportable segments.",
         Font(name="Calibri", size=10)),
        (6, "REVENUE DISAGGREGATION (ASC 606)",
         Font(bold=True, name="Calibri", size=11)),
        (7, "Revenue is disaggregated into TWO categories in the 10-K:",
         Font(name="Calibri", size=10)),
        (8, "  1. Product  — next-gen firewall appliances and other hardware",
         Font(name="Calibri", size=10)),
        (9, "  2. Subscription and Support (S&S)  — cloud subscriptions, software "
           "updates, premium support, maintenance",
         Font(name="Calibri", size=10)),
        (11, "PRODUCT FAMILIES (commercial brands, NOT segments)",
         Font(bold=True, name="Calibri", size=11)),
        (12, "Strata, Prisma, and Cortex are product families — commercial brands "
           "that span both revenue categories:",
         Font(name="Calibri", size=10)),
        (13, "  • Strata  — generates both Product revenue (from firewall appliances) "
           "AND S&S revenue (from subscriptions running on those appliances)",
         Font(name="Calibri", size=10)),
        (14, "  • Prisma  — cloud-delivered SASE/CNAPP; almost entirely S&S",
         Font(name="Calibri", size=10)),
        (15, "  • Cortex  — XDR/XSIAM; almost entirely S&S",
         Font(name="Calibri", size=10)),
        (16, "Calling Strata/Prisma/Cortex 'segments' is incorrect under ASC 280.",
         Font(bold=True, italic=True, name="Calibri", size=10)),
        (18, "CURRENT MODEL SCOPE",
         Font(bold=True, name="Calibri", size=11)),
        (19, "This model forecasts CONSOLIDATED REVENUE only. Disaggregating into "
         ...
         Font(name="Calibri", size=10)),
        (21, "Forecasting Product and Subscription & Support separately is a v2 enhancement. "
...
def build(ticker: str | None = None, db_path: Path | None = None) -> Path:
    ...
    logger.info("Loaded %d IS rows, %d BS rows, %d CF rows (quarterly)",
                len(hist_is), len(hist_bs), len(hist_cf))
    ...
    logger.info("Base assumptions: rev_growth=%.1f%% gross_margin=%.1f%% dso=%.0fd",
    ...
    _build_cash_flow(
        ...
        hist_is, hist_bs, hist_cf, fcst_base, has_inv,
    )
    _build_debt_schedule(wb.create_sheet("Debt_Schedule"), base_ass, hist_bs)
    _build_summary(
        wb.create_sheet("Summary"),
        resolved_ticker, hist_is, hist_cf, fcst_base, ocf_residual,
    )
    if sources_df is not None and len(sources_df) > 0:
        _build_sources(wb.create_sheet("Sources"), sources_df)

    if has_inv:
        _build_revenue_disaggregation(
            wb.create_sheet("Revenue_Disaggregation"), resolved_ticker
        )
