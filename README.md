...
### Validating drafts offline

`src/validate_commentary.py` exposes the same hallucination guard as a
standalone CLI — useful for iterating on prompts or validating a draft
written from a dry-run payload, with no DuckDB warehouse and no Anthropic
API key:
...
Exit code is `0` on a clean draft, `1` on a guard violation (the bad token
is named in stderr), and `2` on bad input. `--payload` accepts either the
raw JSON written by `--dump-payload` or the full dry-run output text — the
loader auto-detects the format.
